...
class GAN(nn.Module):
    ...
    def update(self, real_states):
        for _ in range(1):
            self.optimizer_D.zero_grad()
            with torch.no_grad():
                fake_next_step = self.gen_img(real_states)
            fake_input = fake_next_step.detach()
            real_input = real_states
...
def batch_generator(env, batch_size=64):
    while True:
        batch = list()
        for _ in range(4):
            obs = env.reset()
...
def get_vae():
    ...
    vae = torch.load('../artifacts/minigrid/models/minigrid_random_empty_vae.p')
...
def train(gan, vae, data_gen, conf):
    ...
    for step in trange(conf['training.n_steps']):
        ...
        real_states = vae.encode(batch)

        loss_D_fake, loss_D_real, loss_D, loss_G = gan.update(real_states)
        ...
        if step % verbose == 0:
            with torch.no_grad():
                gen_state = gan.gen_img(real_states[:1])
                gen_state = vae.decode(gen_state)[0]
            curr_state = batch[0]

            writer.add_image('curr state', curr_state.permute(2, 0, 1), step)
            writer.add_image('gen next state', gen_state, step)
...
def main(args=None):
    ...
    env = make_vec_envs(
        lambda env_id: lambda: gen_env(conf, env_id),
    ...
    vae = get_vae()
    vae.to(conf['training.device'])

    datagen = batch_generator(env)
    train(gan, vae, datagen, conf)
