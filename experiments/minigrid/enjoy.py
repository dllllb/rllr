...
def test():
    algos = {
        ...
        'rnd_ppo_keycorridor': (0.62, 1.0),
        'rnd_ppo_putnear': (0.79, 1.0),

        'ppo_dynobs_fixed_seed': (0.0, 0.0),
        'ppo_lava_fixed_seed': (0.0, 0.0),
        'ppo_doorkey_fixed_seed': (0.0, 0.0),
        'ppo_fourrooms_fixed_seed': (0.0, 0.0),
        'ppo_keycorridor_fixed_seed': (0.0, 0.0),
        'ppo_putnear_fixed_seed': (0.0, 0.0),
        'rnd_ppo_fixed_seed': (0.746874988079071, 1.0),
        'rnd_ppo_lava_fixed_seed': (0.0, 0.0),
        'rnd_ppo_doorkey_fixed_seed': (0.0, 0.0),
...
def play(mode, viz, n_episodes):
    ...
    if mode == 'worker':
        ...
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_first_step.hocon')

    elif mode == 'master':
        from train_master import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_second_step.hocon')

    elif mode == 'ssim_worker':
        from train_worker import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_first_step_ssim.hocon')

    elif mode == 'worker_doorkey':
        from train_worker import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_first_step_doorkey.hocon')

    elif mode == 'master_doorkey':
        from train_master import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_second_step_doorkey.hocon')

    elif mode == 'ssim_master':
        from train_master import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_second_step_ssim.hocon')

    elif mode == 'ssim_master_lava':
        from train_master import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_lava_second_step_ssim.hocon')

    elif mode == 'direct_ppo':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_direct_ppo.hocon')
        config['env']['agent_start_pos'] = (1, 1)

    elif mode == 'ppo_dynobs':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_dynobst.hocon')

    elif mode == 'ppo_lava':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_lava.hocon')

    elif mode == 'ppo_doorkey':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_doorkey.hocon')

    elif mode == 'ppo_fourrooms':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_fourrooms.hocon')

    elif mode == 'ppo_keycorridor':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_keycorridor.hocon')

    elif mode == 'ppo_putnear':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_putnear.hocon')

    elif mode == 'rnd_ppo':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo.hocon')

    elif mode == 'rnd_ppo_lava':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_lava.hocon')

    elif mode == 'rnd_ppo_doorkey':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_doorkey.hocon')

    elif mode == 'rnd_ppo_fourrooms':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_fourrooms.hocon')

    elif mode == 'rnd_ppo_keycorridor':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_keycorridor.hocon')

    elif mode == 'rnd_ppo_putnear':
        from train_lang_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_putnear.hocon')

    elif mode == 'ppo_dynobs_fixed_seed':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_dynobst_fixed_seed.hocon')

    elif mode == 'ppo_lava_fixed_seed':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_lava_fixed_seed.hocon')

    elif mode == 'ppo_doorkey_fixed_seed':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_doorkey_fixed_seed.hocon')

    elif mode == 'ppo_fourrooms_fixed_seed':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_fourrooms_fixed_seed.hocon')

    elif mode == 'ppo_keycorridor_fixed_seed':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_keycorridor_fixed_seed.hocon')

    elif mode == 'ppo_putnear_fixed_seed':
        from train_direct_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_ppo_putnear_fixed_seed.hocon')

    elif mode == 'rnd_ppo_fixed_seed':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_fixed_seed.hocon')

    elif mode == 'rnd_ppo_lava_fixed_seed':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_lava_fixed_seed.hocon')

    elif mode == 'rnd_ppo_doorkey_fixed_seed':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_doorkey_fixed_seed.hocon')

    elif mode == 'rnd_ppo_fourrooms_fixed_seed':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_fourrooms_fixed_seed.hocon')

    elif mode == 'rnd_ppo_keycorridor_fixed_seed':
        from train_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_keycorridor_fixed_seed.hocon')

    elif mode == 'rnd_ppo_putnear_fixed_seed':
        state_preproc = lambda x: x['state']
        from train_lang_rnd_ppo import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_rnd_ppo_putnear_fixed_seed.hocon')

    elif mode == 'multi':
        from train_multitask import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_multitask.hocon')

    elif mode == 'ssim_lava_fixed_seed':
        from train_master import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_lava_S9N3_second_step_ssim.hocon')

    elif mode == 'ssim_lava_fixed_go_agent_worker':
        from train_worker import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_lava_S9N3_first_step_ssim_go_agent.hocon')

    elif mode == 'ssim_lava_fixed_go_agent':
        from train_master import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_lava_S9N3_second_step_ssim_go_agent.hocon')

    elif mode == 'goal_keycorridor_worker':
        from train_goal_worker import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_goal_first_step_glyphs_ohe.hocon')

    elif mode == 'goal_keycorridor_master':
        from train_goal_master import gen_env_with_seed
        config = ConfigFactory.parse_file(Path(__file__).parent / 'conf/minigrid_goal_second_step_glyphs_ohe.hocon')
